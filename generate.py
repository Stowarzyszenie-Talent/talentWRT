...
class QueryType(Generic[CT], ABC):
    @abstractmethod
    def hint(self, default: CT | None) -> str:
        pass

    @abstractmethod
    def parse(self, input: str) -> CT:
        pass
...
class OneOf(QueryType[str]):
    ...
    def hint(self, default: str | None) -> str:
        return (
            "{" + ", ".join(f"[{v}]" if v == default else v for v in self.values) + "}"
        )
...
class File(QueryType[TextIO]):
    def hint(self, default: TextIO | None) -> str:
        return f"(path) [{default}]" if default else ""
...
class IPv4Address(QueryType[str]):
    def hint(self, default: str | None) -> str:
        return f"(adres IPv4){f' [{default}]' if default else ''}"
...
class Integer(QueryType[int]):
    def hint(self, default: int | None) -> str:
        return f"(int){f' [{default}]' if default else ''}"
...
if kind == "LO3":
    ...
    i = 0
    while (addr := query("IP routera talent", IPv4Address(), default="")) != "":
